...
public class ControlPanelHelper {
    ...
    public static void setText(ControlPanelArea cp, String text) {
        TextTexture textTexture = new TextTexture(Color.LIGHTGRAY);
        cp.setTexture(textTexture.getTextureForText(text, Color.RED));
    }
     ...
     */
    public static void positionToNearPlaneByGrid(Camera camera, double distanceToNearplane, Dimension gridDimension, Point panelPosition, ControlPanel panel, DimensionF nativePanelSize) {
        double distance = camera.getNear() + distanceToNearplane;
        DimensionF planeSize = camera.getPlaneSize(distance);
        logger.debug("nearPlaneSize=" + planeSize);

        DimensionF gridCellSize = new DimensionF(planeSize.width / gridDimension.getWidth(), planeSize.height / gridDimension.getHeight());
        Vector2 position = new Vector2(gridCellSize.width / 2.0 - planeSize.width / 2.0, gridCellSize.height / 2.0 - planeSize.height / 2.0);
        ...
        logger.debug("");
        panel.getTransform().setPosition(new Vector3(position.getX(), position.getY(), -(distance + 0.0001/*protect againt distance 0**/)));
